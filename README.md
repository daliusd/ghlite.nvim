...
- Neovim 0.12+ includes `nvim.difftool`, used by default for enhanced diff viewing.

- [Diffview.nvim](https://github.com/sindrets/diffview.nvim) and [codediff.nvim](https://github.com/esmuellert/codediff.nvim) remain optional alternatives.
...
```lua
  {
    ...
    config = function()
      require('ghlite').setup({
        ...
        diff_tool = 'auto', -- 'difftool', 'diffview', 'codediff', or 'auto' - tool for GHLitePRDiffview
...
* `ca` to write a top-level PR comment, or quote a PR/review comment when the cursor is on its body
...
This command can use Neovim's built-in `nvim.difftool` (Neovim 0.12+) or an
optional external plugin, according to `diff_tool`:

- `diff_tool = 'auto'` (default): Prefers built-in difftool, then diffview.nvim,
  then codediff.nvim.
- `diff_tool = 'difftool'`: Always uses built-in difftool.
- `diff_tool = 'diffview'` or `diff_tool = 'codediff'`: Uses that plugin.

Built-in difftool compares temporary detached Git worktrees for the base and
head revisions. When the PR is checked out, its current working tree is used
as the head side instead, including uncommitted changes. Temporary worktrees
are removed when Neovim exits.
